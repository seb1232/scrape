...
def select_board(driver):
    ...
    try:
        ...
        options = driver.find_elements(By.CSS_SELECTOR, 'div.selector-list-item-text')
        if not options:
        ...
        try:
            ...
            if 0 <= choice_idx < len(options):
                ...
                time.sleep(3)
                # After clicking, fetch the board name from the header (not the dropdown)
                try:
                    board_name = driver.find_element(By.CSS_SELECTOR, 'div.feedback-board-container-header div.selector-button span').text.strip()
                except Exception:
                    board_name = ''
                print(f"Selected board: {board_name if board_name else '[unknown]'}")
                return board_name
            else:
...
def add_new_feedback(column_title, feedback_text):
    ...
    try:
        driver.get(RETRO_URL)
        print("Opening retrospective board...")
        time.sleep(30)  # Wait for manual login or page load
        login_if_needed(driver)
        time.sleep(5)
        ...
        try:
            ...
            print(f"Could not access the board iframe: {e}")
            return f"Could not access the board iframe: {e}"

        # ✅ Step 2: Let user select board
        selected_board = select_board(driver)
        if selected_board is None:
            print("No board selected. Please try again.")
            return "Board selection cancelled or failed."
        if not selected_board.strip():
            print("Warning: Board name is empty, but proceeding to add feedback anyway.")
        else:
            print(f"Proceeding to add feedback to board: {selected_board}")
        time.sleep(2)  # Wait for board to load

        # Step 3: Add the feedback
        columns = driver.find_elements(By.CSS_SELECTOR, 'div.feedback-column')
        found_column = False
        for col in columns:
            try:
                ...
                if col_title.lower() == column_title.lower():
                    ...
                    try:
                        ...
                        for btn in col.find_elements(By.CSS_SELECTOR, 'button'):
                            if btn.is_enabled():
                                btn.click()
                                print("Feedback added with fallback button!")
                                time.sleep(3)
                                return "Feedback added successfully (fallback)!"
                        print("Could not find a save button to click")
                        return "Could not find a save button to click"
            except Exception as e:
                print(f"Error finding column or adding feedback: {e}")
                continue
        if not found_column:
            print(f"Could not find column '{column_title}' in board '{selected_board}'")
            return f"Could not find column '{column_title}' in board '{selected_board}'"
    except Exception as e:
...
def analyze_feedback_with_ollama(cards, question):
    ...
    # Check for 'created by' or 'mentioning' a person pattern (force direct search bypassing AI)
    import re
    person_match = re.search(r'(?:created by|mentioning|about|for) ([\w, .-]+)', question, re.IGNORECASE)
    if person_match:
        person_name = person_match.group(1).strip()
        return search_feedback_by_person(cards, person_name)
    try:
...
def analyze_feedback_question(cards, question):
    ...
    # Check for 'created by' or 'mentioning' a person pattern
    import re
    person_match = re.search(r'(?:created by|mentioning|about|for) ([\w, .-]+)', question, re.IGNORECASE)
    if person_match:
        person_name = person_match.group(1).strip()
...
def scrape_all_boards_and_train_ai():
    ...
    try:
        ...
        print("- Add new feedback: 'Add feedback to [column]: [text]' (you will be prompted to select a board)")
        print("- Edit feedback: 'Edit feedback in [column]: [old text] -> [new text]' (AI-powered, you will be prompted to select a board)")
...
def scrape_all_boards_all_tabs_and_train_ai():
    """Scrape all boards, all tabs (Collect, Vote, Act), and train AI. Now logs conversation."""
    driver.get(RETRO_URL)
    ...
    try:
        ...
        with open(conversation_log_path, 'a', encoding='utf-8') as logf:
            logf.write(f"Session started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        while True:
            question = input("\n🤖 Ask your question: ").strip()
            if question.lower() == 'exit':
                break
...
def edit_feedback_ai():
    """AI-powered feedback editing in user-selected board and column."""
    global driver
    driver.get(RETRO_URL)
    print("Opening retrospective board for editing...")
    time.sleep(30)
    login_if_needed(driver)
    time.sleep(5)
    # Step 1: Switch to iframe
    ...
    if selected_board is None:
        print("No board selected. Please try again.")
        return "Board selection cancelled or failed."
    print(f"Proceeding to edit feedback in board: {selected_board}")
    time.sleep(2)
    # Step 3: Get edit request from user
    edit_request = input("Describe the feedback you want to edit (e.g., 'Edit feedback in [column]: [old text] -> [new text]'): ").strip()
    # Step 4: Use AI to parse edit request
    prompt = f'''The user wants to edit an existing feedback card in a retrospective board. Please analyze their request and extract:
1. The column/category where the feedback is located (like "What went well", etc.)
2. The current feedback text (to identify the card)
3. The new feedback text (to update the card)

User request: {edit_request}

Please respond in this exact format:
COLUMN: [column name]
OLD: [current feedback text]
NEW: [new feedback text]

If you cannot determine the column, old, or new feedback, ask for clarification.'''
    try:
        response = ollama.chat(model='deepseek-coder:6.7b', messages=[{'role': 'user', 'content': prompt}])
        ai_response = response['message']['content']
        lines = ai_response.split('\n')
        column = old_text = new_text = None
        for line in lines:
            if line.startswith('COLUMN:'):
                column = line.replace('COLUMN:', '').strip()
        ...
        if not (column and old_text and new_text):
            print(f"AI needs more information.\n{ai_response}")
            return
        print(f"\n🤖 AI wants to edit feedback:")
        print(f"   Column: {column}")
        print(f"   Old: {old_text}")
        print(f"   New: {new_text}")
        confirm = input("\nProceed with editing this feedback? (y/n): ").strip().lower()
        if confirm not in ('y', 'yes'):
            print("❌ Feedback edit cancelled.")
            return
        # Step 5: Find and edit the feedback card
        columns = driver.find_elements(By.CSS_SELECTOR, 'div.feedback-column')
        found_column = False
        def normalize(text):
            return ' '.join(text.lower().split()) if text else ''
        norm_old_text = normalize(old_text)
        for col in columns:
            try:
                col_title = col.find_element(By.CSS_SELECTOR, '.feedback-column-title h2').text.strip()
                if col_title.lower() == column.lower():
                    found_column = True
                    print(f"Found column: {col_title}")
                    card_container = col.find_element(By.CSS_SELECTOR, '.feedback-column-content')
                    cards = card_container.find_elements(By.CSS_SELECTOR, 'div.feedbackItem')
                    for card in cards:
                        ...
                        if not feedback:
                            try:
                                ...
                                continue
                        # Flexible matching: case-insensitive, ignore extra spaces, allow partial match
                        if feedback and (normalize(feedback) == norm_old_text or norm_old_text in normalize(feedback)):
                            # Found the card to edit
                            try:
                                textarea = None
                                contenteditable = None
                                input_field = None
                                # Try to find the editable <p> and click it to activate editing
                                try:
                                    editable_p = card.find_element(By.CSS_SELECTOR, 'p.editable-text')
                                    webdriver.ActionChains(driver).move_to_element(editable_p).double_click(editable_p).perform()
                                    time.sleep(2)
                                    # After activating, try to find textarea
                                    try:
                                        textarea = card.find_element(By.CSS_SELECTOR, 'textarea.ms-TextField-field')
                                    ...
                                    pass
                                # Fallback: try previous double-click on card
                                if not textarea:
                                    webdriver.ActionChains(driver).double_click(card).perform()
                                    time.sleep(2)
                                    try:
                                        textarea = card.find_element(By.CSS_SELECTOR, 'textarea.ms-TextField-field')
                                        if textarea.is_displayed() and textarea.is_enabled():
                                ...
                                if not textarea:
                                    ...
                                    try:
                                        contenteditable = card.find_element(By.CSS_SELECTOR, '[contenteditable="true"]')
                                    except Exception:
                                        pass
                                if not textarea and not contenteditable:
                                    # Try to find input field
                                    try:
                                        input_field = card.find_element(By.CSS_SELECTOR, 'input[type="text"]')
                                    except Exception:
                                        pass
                                if not textarea and not contenteditable and not input_field:
                                    # Try clicking an Edit button if present
                                    try:
                                        edit_btn = None
                                        for btn in card.find_elements(By.CSS_SELECTOR, 'button'):
                                        ...
                                        if edit_btn:
                                            edit_btn.click()
                                            time.sleep(1)
                                            # Retry all editable field searches
                                            try:
                                                textarea = card.find_element(By.CSS_SELECTOR, 'textarea.ms-TextField-field')
                                            except Exception:
                                                pass
                                        ...
                                        pass
                                # Now, try to edit using whichever field was found
                                if textarea:
                                    textarea.clear()
                                    # Extra: ensure field is empty (for React/JS UIs)
                                    ...
                                    contenteditable.send_keys(new_text)
                                elif input_field:
                                    input_field.clear()
                                    input_field.send_keys(new_text)
                                else:
                                    # Print card HTML for debugging
                                    print("Could not find an editable field. Card HTML:\n" + card.get_attribute('outerHTML'))
                                    return "Could not find an editable field (textarea, contenteditable, or input) for this feedback card. Card HTML printed for debugging. Please check the UI or update the selector."
                                # Save
                                try:
                                    save_btn = card.find_element(By.CSS_SELECTOR, 'button.ms-Button--primary')
                                    save_btn.click()
                                except Exception:
                                    for btn in card.find_elements(By.CSS_SELECTOR, 'button'):
                                        if btn.is_enabled():
                                            btn.click()
                                            break
                                print("Feedback edited!")
                                time.sleep(3)
                                return "Feedback edited successfully!"
                            except Exception as e:
                                print(f"Error editing feedback: {e}")
                                return f"Error editing feedback: {e}"
                    print(f"Could not find feedback card with the specified text in column '{column}'")
                    return f"Could not find feedback card with the specified text in column '{column}'"
            except Exception as e:
                print(f"Error finding column or editing feedback: {e}")
                continue
        if not found_column:
            print(f"Could not find column '{column}' in board '{selected_board}'")
            return f"Could not find column '{column}' in board '{selected_board}'"
    except Exception as e:
        print(f"Error in AI feedback edit: {e}")
        return f"Error in AI feedback edit: {e}"


def search_feedback_by_person(cards, person_name):
    """Return all feedback cards mentioning the given person's name (case-insensitive, partial match, ignore punctuation and spaces)."""
    import re
    # Normalize the person_name: remove punctuation, lower, strip spaces
    def normalize(text):
        return re.sub(r'[^a-z0-9]', '', text.lower())
    norm_person = normalize(person_name)
    matches = []
    for card in cards:
        norm_text = normalize(card['text'])
        if norm_person and norm_person in norm_text:
            matches.append(card)
    if not matches:
        return f"No feedback cards found mentioning '{person_name}'."
    result = f"📋 Feedback cards mentioning '{person_name}':\n"
    result += "=" * 50 + "\n"
    for i, card in enumerate(matches, 1):
        ...
        date = card.get('date', '')
        votes = card.get('votes', 0)
        result += f"\n#{i} [Board: {board}] [Tab: {tab}] [Column: {column}]"
        if date:
            result += f" [Date: {date}]"
        if votes:
            result += f" [Votes: {votes}]"
        result += f"\n    {card['text'][:200]}\n"
    return result

def get_conversation_log_path():
    """Return a unique log file path for the current session."""
    log_dir = os.path.join(os.getcwd(), 'ai_conversations')
    os.makedirs(log_dir, exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    return os.path.join(log_dir, f'conversation_{timestamp}.txt')
...
Options:
8. (display)Scrape Current Board: Opens browser, lets you select a retrospective board, switches to iframe, 
   extracts all feedback cards from all columns, displays card text with column organization, 
   handles login automation, extracts dates from feedback text, keeps browser open for continued use

9. Add New Feedback: Opens browser, lets you select board and column, uses web automation to click 
   'Add new feedback' button, fills textarea with your text, clicks save button, handles iframe 
   switching, provides success/error feedback, supports all column types (What went well, etc.)

10. Scrape & AI Chat: Scrapes all feedback from current board,ADD or edit feedback in user choice of board, extracts votes/dates/text, sets up 
    Ollama AI model (deepseek-coder:6.7b), creates interactive Q&A session, supports commands like 
    'summary', 'trends', 'stats', handles vote filtering, AI-powered feedback creation, searches 
    by keywords/dates, provides intelligent analysis and insights

11. Scrape ALL boards and ALL tabs (Collect/Vote/Act) and start comprehensive AI chat(cant add/edit,But any questions about the retrospecitce)
""")
    option = input("Choose an option (8-11): ").strip()
    if option == '8':
        ...
        while True:
            ...
            if question.lower() == 'exit':
                ...
                continue
            elif question.lower().startswith('edit feedback'):
                result = edit_feedback_ai()
                print(f"\n{result}")
                # Log edit feedback interaction
                with open(conversation_log_path, 'a', encoding='utf-8') as logf:
                    logf.write(f"User: {question}\nAI: {result}\n\n")
                continue
        ...
        print("Please choose a valid option (8-11).")
